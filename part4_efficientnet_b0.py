...
from src.models.efficientnet_b0 import build_efficientnet_b0, count_parameters
...
def load_class_mapping(mapping_path: Path) -> dict[str, int]:
    """Load the fixed class mapping created during data preparation."""
...
def make_loader(
    ...
    """Create a loader from an existing split without changing the split."""
    dataset = LeafDataset(
        samples=samples,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Train EfficientNet-B0 using the fixed training and validation splits."
    )
    parser.add_argument(
        "--config",
        type=Path,
        default=Path("configs/efficientnet_b0_frozen.yaml"),
    )
...
def main() -> None:
    ...
    class_to_idx = load_class_mapping(Path("data/metadata/class_to_idx.json"))
    ...
    train_loader = make_loader(
        samples=train_samples, training=True, **loader_arguments
    )
    val_loader = make_loader(samples=val_samples, training=False, **loader_arguments)
    ...
    for epoch in range(1, epochs + 1):
        ...
        if val_accuracy > best_val_accuracy:
            ...
            checkpoint_settings = {
                ...
                "trainable_scope": model_config["trainable_scope"],
                "seed": seed,
                "validation_only_extension": True,
            }
            save_checkpoint(
                best_model_path,
                model=model,
    ...
    print("EfficientNet-B0 is a validation-only extension; test was not accessed.")
